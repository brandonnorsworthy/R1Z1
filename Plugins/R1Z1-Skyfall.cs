...
{
    [Info("Skyfall", "Colon Blow", "1.0.13")]
    class Skyfall : RustPlugin
    {

        // Corrected Ground Release Height for player chute
        // Added config option to easily allow changing ground release height.
        ...
        {
            ConVar.Chat.Broadcast("dropping in " + ChaosDropCountdown.ToString("F0") + " seconds", "R1Z1", "#FF0000");
            timer.Once(ChaosDropCountdown * 0.25f, () => ConVar.Chat.Broadcast("dropping in " + (ChaosDropCountdown * 0.75f).ToString("F0") + " seconds", "R1Z1", "#FF0000"));
            timer.Once(ChaosDropCountdown * 0.50f, () => ConVar.Chat.Broadcast("dropping in " + (ChaosDropCountdown * 0.50f).ToString("F0") + " seconds", "R1Z1", "#FF0000"));
            timer.Once(ChaosDropCountdown * 0.75f, () => ConVar.Chat.Broadcast("dropping in " + (ChaosDropCountdown * 0.25f).ToString("F0") + " seconds", "R1Z1", "#FF0000"));
            timer.Once(ChaosDropCountdown, () =>
            {
                PrintWarning("R1Z1: Chaos Drop Has Been Activated...");
                DoChaosDrop();
            });
        }
        ...
        void ActivateJumpPlane(BasePlayer player)
        {
            ...
            if (jumpplane)
            {
                SendReply(player, "Drop Countdown 3...");
                timer.Once(1f, () => SendReply(player, "Drop Countdown 2..."));
                timer.Once(2f, () => SendReply(player, "Drop Countdown 1..."));
                timer.Once(3f, () =>
                {
        ...
        class SkyfallRespawnButton : MonoBehaviour
        {
            ...
            public void RespawnButton(BasePlayer player)
            {
                ...
                string textstring = "Use Drop \n Respawn";
                if (cooldownlist.Contains(player.userID)) { colorstring = "0.30 0.25 0.14 0.5"; textstring = "Drop Respawn \n Unavailable"; }
